...
# logo = r"./Talk_to_your_data/botimg.png"
...
#                 response, no_of_characters_used = chatbot_agent.pdf_chat(prompt)
...
import streamlit as st
from chatbot_agent import chatbot_agent
from tools.pdf_reader3 import pdf_loader,llmGen
# Clears chat history displayed on the chatbot screen
uploaded=False
def clear_chat_history():
    st.session_state.messages = [
...
def main():
    ...
    with st.sidebar:
        ...
        google = st.text_input("Enter your OpenAI API Key", type="password")
        print(api_key)
        # Check if API key is entered
        if api_key:
            st.session_state.api_key = api_key
        if 'api_key' in st.session_state:
            llmGen.setApiKey(st.session_state.api_key)
    ...
    # Use the API key stored in session state
    if 'api_key' in st.session_state:
        chatbot_agent.llm_model(llm_model, temperature, output_context_size,key=st.session_state.api_key,docStatus=uploaded)
